...
router.get('/detail/:id/:sort', (req, res) => {
  ...
  switch (sort) {
    ...
    case 'new': connection.query(`select * from product inner join admin_store on product.user_key = admin_store.user_key inner join user on user.user_key = admin_store.user_key where admin_store.store_key = ${id} ORDER BY product.product_post_date DESC;`, (err, row, field) => {
      ...
      break;
    default: connection.query(`select * from product inner join admin_store on product.user_key = admin_store.user_key inner join user on user.user_key = admin_store.user_key where admin_store.store_key = ${id}`, (err, row, field) => {
      res.send(row);
    })
      break;
  }
